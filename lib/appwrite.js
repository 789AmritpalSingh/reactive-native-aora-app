...
export const likeVideo = async (videoId, userId) => {
  try {
    ...
    const updatedLikes = video.likes ? [...video.likes.map(like => like.$id ? like.$id : like), userId] : [userId];

    await databases.updateDocument(databaseId, videoCollectionId, videoId, { likes: updatedLikes });
  } catch (error) {
    throw new Error(error);
  }
};
...
export const unlikeVideo = async (videoId, userId) => {
  try {
    ...
    // Ensure likes is an array of user IDs and filter out the user ID
    const updatedLikes = video.likes ? video.likes.filter(like => (like.$id ? like.$id : like) !== userId) : [];
...
export const getLikesForVideo = async (videoId) => {
  try {
    const video = await databases.getDocument(databaseId, videoCollectionId, videoId);
    // Ensure likes is an array of user IDs
    const likes = video.likes ? video.likes.map(like => like.$id) : [];
    return likes;
  } catch (error) {
    throw new Error(error);
  }
...
export const getVideosLikedByUser = async (userId) => {
  try {
    const response = await databases.listDocuments(databaseId, videoCollectionId);
    
    // Log the documents to inspect the structure
    console.log('Response documents - ', response.documents);

    // Check and filter videos where the likes array includes the userId
    const likedVideos = response.documents.filter(video => {
      console.log(`Video ID: ${video.$id}, Likes:`, video.likes);
      return video.likes && video.likes.some(like => {
        console.log('Like object:', like);
        return like === userId || (like.$id && like.$id === userId);
      });
    });
